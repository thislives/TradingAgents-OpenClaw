...
class ConditionalLogic:
    ...
    def should_continue_risk_analysis(self, state: AgentState) -> str:
        ...
        Round structure: Aggressive → Conservative → Neutral → (next round) Aggressive → ...
        count is incremented at the END of each speaker's turn.
        With max_risk_discuss_rounds=1: Aggressive(1) → Conservative(2) → Neutral(3) → 
        Aggressive(4) → then exit. So we exit when count > 3, not >= 3.
        """
        if (
            state["risk_debate_state"]["count"] > 3 * self.max_risk_discuss_rounds
        ):  # count=4 when Aggressive finishes 2nd round; 3 speakers × 1 round + 1 = 4
